...
export const Photos = (props) => {
    const {setSelectedPhoto} = props

    return (
        <ul className="flex flex-col">
            {photos.map((photo) => {
                return <li className="mb-16" key={photo.label} onClick={() => setSelectedPhoto(photo)}>
                    <img className="rounded object-cover" src={photo.url} alt={photo.label} />
                    <h3 className="px-2 mt-2 bold text-center">{photo.label}</h3>
                </li>
            })}
        </ul>
    )
